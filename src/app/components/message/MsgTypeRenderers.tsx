...
export function MImage({ content, renderImageContent, outlined }: MImageProps) {
  ...
  return (
    <Attachment outlined={outlined}>
      ...
      >
        {renderImageContent({
          body: content.body || t('message.fallback_names.image'),
          info: imgInfo,
...
export function MVideo({ content, renderAsFile, renderVideoContent, outlined }: MVideoProps) {
  ...
  const filename = content.filename ?? content.body ?? t('message.fallback_names.video');
  ...
  return (
    <Attachment outlined={outlined}>
      ...
      >
        {renderVideoContent({
          body: content.body || t('message.fallback_names.video'),
          info: videoInfo,
          mimeType: safeMimeType,
          url: mxcUrl,
          encInfo: content.file,
...
export function MAudio({ content, renderAsFile, renderAudioContent, outlined }: MAudioProps) {
  ...
  const filename = content.filename ?? content.body ?? t('message.fallback_names.audio');
  return (
    <Attachment outlined={outlined}>
      <AttachmentHeader>
        <FileHeader
...
export function MFile({ content, renderFileContent, outlined }: MFileProps) {
  ...
  return (
    <Attachment outlined={outlined}>
      <AttachmentHeader>
        <FileHeader
          body={content.filename ?? content.body ?? t('message.fallback_names.unnamed_file')}
      ...
      <AttachmentBox>
        <AttachmentContent>
          {renderFileContent({
            body: content.filename ?? content.body ?? t('message.fallback_names.file'),
            info: fileInfo ?? {},
            mimeType: fileInfo?.mimetype ?? FALLBACK_MIMETYPE,
            url: mxcUrl,
            encInfo: content.file,
...
export function MLocation({ content }: MLocationProps) {
  ...
  return (
    <Box direction="Column" alignItems="Start" gap="100">
      ...
      >
        <Text size="B300">{t('message.location.open_location')}</Text>
      </Chip>
    </Box>
  );
}
...
export function MSticker({ content, renderImageContent }: MStickerProps) {
  ...
  return (
    ...
    >
      {renderImageContent({
        body: content.body || t('message.fallback_names.sticker'),
        info: imgInfo,
        mimeType: imgInfo?.mimetype,
        url: mxcUrl,
        encInfo: content.file,
