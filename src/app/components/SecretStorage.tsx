  ...
  >(
    useCallback(
      async (passphrase, salt, iterations, bits) => {
        ...
        if (!match) {
          throw new Error(t('secret_storage.invalid_passphrase_error'));
        }
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="100">
      <Box gap="200" alignItems="End">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('secret_storage.recovery_passphrase_label')}</Text>
          <PasswordInput
        ...
        <Box shrink="No" gap="200">
          ...
          >
            <Text as="span" size="B400">
              {t('secret_storage.verify_button')}
            </Text>
          </Button>
        </Box>
      </Box>
...
export function SecretStorageRecoveryKey({
  ...
  const [driveKeyState, submitRecoveryKey] = useAsyncCallback<Uint8Array, Error, [string]>(
    useCallback(
      async (recoveryKey) => {
        ...
        if (!match) {
          throw new Error(t('secret_storage.invalid_key_error'));
        }
  ...
  return (
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="100">
      <Box gap="200" alignItems="End">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('secret_storage.recovery_key_label')}</Text>
          <PasswordInput
