  ...
  return (
    <Box direction="Column" gap="400">
      ...
      <Button variant="Secondary" fill="Soft" onClick={onClose}>
        <Text size="B400">{t('device_verification.close_button')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationWaitAccept() {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('device_verification.wait_accept_prompt')}</Text>
      <WaitingMessage message={t('device_verification.waiting_accept_message')} />
    </Box>
  );
}
...
function VerificationAccept({ onAccept }: VerificationAcceptProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('device_verification.accept_prompt')}</Text>
      <Button
        variant="Primary"
        fill="Solid"
        onClick={accept}
        ...
        disabled={accepting}
      >
        <Text size="B400">{t('device_verification.accept_button')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationWaitStart() {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('device_verification.wait_start_message')}</Text>
      <WaitingMessage message={t('device_verification.waiting_response_message')} />
    </Box>
  );
}
...
function AutoVerificationStart({ onStart }: VerificationStartProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <WaitingMessage message={t('device_verification.starting_sas_message')} />
    </Box>
  );
}
...
function CompareEmoji({ sasData }: { sasData: ShowSasCallbacks }) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('device_verification.compare_emoji_prompt')}</Text>
      <Box
      ...
      <Box direction="Column" gap="200">
        ...
        >
          <Text size="B400">{t('device_verification.match_button')}</Text>
        </Button>
        <Button
          variant="Primary"
          fill="Soft"
          ...
          disabled={confirming}
        >
          <Text size="B400">{t('device_verification.no_match_button')}</Text>
        </Button>
      </Box>
    </Box>
  );
...
function VerificationDone({ onExit }: VerificationDoneProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <div>
        <Text>{t('device_verification.verified_message')}</Text>
      </div>
      <Button variant="Primary" fill="Solid" onClick={onExit}>
        <Text size="B400">{t('device_verification.okay_button')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationCanceled({ onClose }: VerificationCanceledProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('device_verification.canceled_message')}</Text>
      <Button variant="Secondary" fill="Soft" onClick={onClose}>
        <Text size="B400">{t('device_verification.close_button')}</Text>
      </Button>
    </Box>
        ...
        >
          <Dialog variant="Surface">
            <Header style={DialogHeaderStyles} variant="Surface" size="500">
              <Box grow="Yes">
                <Text size="H4">{t('device_verification.title')}</Text>
              </Box>
              ...
              {phase === VerificationPhase.Started &&
                (request.verifier ? (
                  ...
                  <VerificationUnexpected
                    message={t('device_verification.unexpected_verifier_missing_error')}
