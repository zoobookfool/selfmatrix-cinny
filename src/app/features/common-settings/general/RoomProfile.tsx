    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="400">
      <Box gap="400">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('room_settings.general.profile.avatar_label')}</Text>
          {uploadAtom ? (
            ...
            <Box gap="200">
              ...
              >
                <Text size="B300">{t('room_settings.general.profile.upload_button')}</Text>
              </Button>
              {!roomAvatar && avatar && (
                <Button
                  type="button"
                ...
                >
                  <Text size="B300">{t('room_settings.general.profile.reset_button')}</Text>
                </Button>
              )}
              {roomAvatar && (
                <Button
                ...
                >
                  <Text size="B300">{t('room_settings.general.profile.remove_button')}</Text>
                </Button>
              )}
            </Box>
          )}
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('room_settings.general.profile.name_label')}</Text>
        <Input
          name="nameInput"
          defaultValue={name}
          variant="Secondary"
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('room_settings.general.profile.topic_label')}</Text>
        <TextArea
          name="topicTextArea"
          defaultValue={topic}
          variant="Secondary"
      ...
      <Box gap="300">
        ...
        >
          <Text size="B300">{t('room_settings.general.profile.save_button')}</Text>
        </Button>
        <Button
          type="reset"
          onClick={onClose}
          ...
          radii="300"
        >
          <Text size="B300">{t('room_settings.general.profile.cancel_button')}</Text>
        </Button>
      </Box>
    </Box>
  );
...
export function RoomProfile({ permissions }: RoomProfileProps) {
  ...
  return (
    <Box direction="Column" gap="100">
      <Text size="L400">{t('room_settings.general.profile.title')}</Text>
      <SequenceCard
            ...
            <Box grow="Yes" direction="Column" gap="300">
              <Box direction="Column" gap="100">
                <Text className={BreakWord} size="H5">
                  {name ?? t('room_settings.general.profile.unknown_name')}
                </Text>
                {topic && (
              ...
              {canEdit && (
                <Box gap="200">
                  <Chip
                    ...
                    outlined
                  >
                    <Text size="B300">{t('room_settings.general.profile.edit_button')}</Text>
                  </Chip>
                </Box>
              )}
            </Box>
