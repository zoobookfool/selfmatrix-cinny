...
function BackupStatus({ enabled }: BackupStatusProps) {
  ...
  return (
    <Box as="span" gap="100" alignItems="Center">
      ...
      >
        {enabled ? t('backup_restore.connected_status') : t('backup_restore.disconnected_status')}
      </Text>
    </Box>
  );
}
  ...
  return (
    <Box as="span" gap="100" alignItems="Center">
      ...
      <Text as="span" size="L400" style={{ color: color.Primary.Main }}>
        {t('backup_restore.syncing_status', { count })}
      </Text>
    </Box>
  );
}
  ...
  return (
    <Box grow="Yes" gap="200" alignItems="Center">
      <Badge variant="Secondary" fill="Solid" radii="300">
        <Text size="L400">{t('backup_restore.restoring_fetching_status')}</Text>
      </Badge>
...
function BackupProgress({ total, downloaded }: BackupProgressProps) {
  ...
  return (
    <Box grow="Yes" gap="200" alignItems="Center">
      <Badge variant="Secondary" fill="Solid" radii="300">
        <Text size="L400">
          {t('backup_restore.restoring_progress_status', {
    ...
    <Box direction="Column">
      {trust.matchesDecryptionKey ? (
        <Text size="T200" style={{ color: color.Success.Main }}>
          <b>{t('backup_restore.trusted_decryption_key_message')}</b>
        </Text>
      ) : (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('backup_restore.untrusted_decryption_key_message')}</b>
        </Text>
      )}
      {trust.trusted ? (
        <Text size="T200" style={{ color: color.Success.Main }}>
          <b>{t('backup_restore.trusted_signature_message')}</b>
        </Text>
      ) : (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('backup_restore.untrusted_signature_message')}</b>
        </Text>
      )}
    </Box>
  );
  ...
  return (
                  ...
                  <Box direction="Column" gap="100">
                    <Box direction="Column" gap="200">
                      <InfoCard
                        ...
                        title={t('backup_restore.details_title')}
                        description={
                          <>
                            <span>
                              {t('backup_restore.version_label', {
                                version: backupInfo?.version ?? t('backup_restore.unknown_value'),
                              })}
                            </span>
                            <br />
                            <span>
                              {t('backup_restore.keys_label', {
                                keyCount: backupInfo?.count ?? t('backup_restore.unknown_value'),
                              })}
                            </span>
                          </>
                        }
                    ...
                    >
                      <Text size="B300">{t('backup_restore.restore_button')}</Text>
                    </Button>
                  </Box>
                </Menu>
              </FocusTrap>
    ...
    >
      ...
      {!backupEnabled && backupInfo === null && (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('backup_restore.no_backup_error')}</b>
        </Text>
      )}
