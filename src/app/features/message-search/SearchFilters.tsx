...
function OrderButton({ order, onChange }: OrderButtonProps) {
  ...
  return (
        ...
        >
          <Menu variant="Surface">
            <Header size="300" variant="Surface" style={{ padding: `0 ${config.space.S300}` }}>
              <Text size="L400">{t('search.filters.sort_by')}</Text>
            </Header>
            ...
            <div style={{ padding: config.space.S100 }}>
              ...
              >
                <Text size="T300">{t('search.filters.sort_recent')}</Text>
              </MenuItem>
              <MenuItem
                ...
                aria-pressed={rankOrder}
              >
                <Text size="T300">{t('search.filters.sort_relevance')}</Text>
              </MenuItem>
            </div>
          </Menu>
        </FocusTrap>
    ...
    >
      ...
      >
        {rankOrder ? (
          <Text size="T200">{t('search.filters.sort_relevance')}</Text>
        ) : (
          <Text size="T200">{t('search.filters.sort_recent')}</Text>
        )}
      </Chip>
    </PopOut>
  );
...
function SelectRoomButton({ roomList, selectedRooms, onChange }: SelectRoomButtonProps) {
  ...
  return (
        ...
        >
          <Menu variant="Surface" style={{ width: toRem(250) }}>
            <Box direction="Column" style={{ maxHeight: toRem(450), maxWidth: toRem(300) }}>
              ...
              >
                <Text size="L400">{t('search.filters.search_label')}</Text>
                <Input
              ...
              <Scroll ref={scrollRef} size="300" hideTrack>
                ...
                >
                  {!searchResult && <Text size="L400">{t('search.filters.rooms_label')}</Text>}
                  {searchResult && (
                    <Text size="L400">
                      {t('search.filters.rooms_for_query', { query: searchResult.query })}
                    </Text>
                  )}
                  {searchResult && searchResult.items.length === 0 && (
                    <Text style={{ padding: config.space.S400 }} size="T300" align="Center">
                      {t('search.filters.no_match_found')}
                    </Text>
                  )}
                  <div
                    style={{
              ...
              <Box shrink="No" direction="Column" gap="100" style={{ padding: config.space.S200 }}>
                <Button size="300" variant="Secondary" radii="300" onClick={handleSave}>
                  {localSelected && localSelected.length > 0 ? (
                    <Text size="B300">
                      {t('search.filters.save_with_count', { count: localSelected.length })}
                    </Text>
                  ) : (
                    <Text size="B300">{t('search.filters.save')}</Text>
                  )}
                </Button>
                <Button
                  size="300"
                ...
                >
                  <Text size="B300">{t('search.filters.deselect_all')}</Text>
                </Button>
              </Box>
            </Box>
          </Menu>
    ...
    >
      ...
      >
        <Text size="T200">{t('search.filters.select_rooms')}</Text>
      </Chip>
    </PopOut>
  );
}
...
export function SearchFilters({
  ...
  return (
    <Box direction="Column" gap="100">
      <Text size="L400">{t('search.filters.filter')}</Text>
      <Box gap="200" wrap="Wrap">
        ...
        {allowGlobal && (
          ...
          >
            <Text size="T200">{t('search.filters.global')}</Text>
          </Chip>
        )}
        <Line
