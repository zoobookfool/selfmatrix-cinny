...
function EmailErrorDialog({
  ...
  const handleFormSubmit: FormEventHandler<HTMLFormElement> = (evt) => {
    ...
    const emailValue = retryEmailInput.value;
    onRetry(emailValue);
  };
      ...
      >
        <Box direction="Column" gap="100">
          ...
          <Text as="label" size="L400" style={{ paddingTop: config.space.S400 }}>
            {t('uia_stages.email.email_label')}
          </Text>
          <Input
...
export function EmailStageDialog({
  ...
  if (emailTokenState.status === AsyncStatus.Loading) {
    return (
      <Box direction="Column" alignItems="Center" gap="400">
        ...
        <Text style={{ color: color.Secondary.Main }}>{t('uia_stages.email.sending_message')}</Text>
      </Box>
    );
  }
  ...
  if (emailTokenState.status === AsyncStatus.Error) {
    return (
      <EmailErrorDialog
        title={emailTokenState.error.errcode ?? t('uia_stages.email.error_fallback_title')}
        message={
          emailTokenState.error?.data?.error ??
          emailTokenState.error.message ??
          t('uia_stages.email.error_fallback_message')
        }
    ...
    return (
      <Dialog>
        <Box style={{ padding: config.space.S400 }} direction="Column" gap="400">
          <Box direction="Column" gap="100">
            <Text size="H4">{t('uia_stages.email.sent_title')}</Text>
            <Text>
              {t('uia_stages.email.sent_message', { email: emailTokenState.data.email })}
            </Text>
          ...
          <Button variant="Primary" onClick={() => handleSubmit(emailTokenState.data.result.sid)}>
            <Text as="span" size="B400">
              {t('uia_stages.email.continue_button')}
            </Text>
          </Button>
        </Box>
      </Dialog>
  ...
  if (!email) {
    return (
      <EmailErrorDialog
        title={t('uia_stages.email.provide_email_title')}
        message={t('uia_stages.email.provide_email_message')}
        onRetry={handleEmailSubmit}
        onCancel={onCancel}
      />
    );
