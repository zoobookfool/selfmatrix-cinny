...
export const useUsageStr = (): ((usage: ImageUsage[]) => string) => {
  ...
  const getUsageStr = (usage: ImageUsage[]): string => {
    ...
    if (sticker && emoticon) return t('image_pack_view.usage.both');
    if (sticker) return t('image_pack_view.usage.sticker');
    if (emoticon) return t('image_pack_view.usage.emoji');
    return t('image_pack_view.usage.both');
  };
  return getUsageStr;
};
