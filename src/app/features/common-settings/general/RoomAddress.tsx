...
export function RoomPublishedAddresses({ permissions }: RoomPublishedAddressesProps) {
  ...
  return (
    ...
    >
      <SettingTile
        title={t('room_settings.general.address.published.title')}
        description={
          <span>
            <Trans i18nKey="room_settings.general.address.published.description">
              <b>Public</b>
            </Trans>
          </span>
        }
      ...
      <CutoutCard variant="Surface" style={{ padding: config.space.S300 }}>
        {publishedAliases.length === 0 ? (
          <Box direction="Column" gap="100">
            <Text size="L400">{t('room_settings.general.address.published.empty_title')}</Text>
            <Text size="T200">
              {t('room_settings.general.address.published.empty_description')}
            </Text>
          </Box>
        ) : (
                ...
                <Box grow="Yes" gap="Inherit" alignItems="Center">
                  ...
                  {alias === canonicalAlias && (
                    <Badge variant="Success" fill="Solid" size="500">
                      <Text size="L400">{t('room_settings.general.address.published.main_badge')}</Text>
                    </Badge>
                  )}
                </Box>
                {canEditCanonical && (
                  <Box shrink="No" gap="100">
                    {alias === canonicalAlias ? (
                      ...
                      >
                        <Text size="B300">{t('room_settings.general.address.published.unset_main_button')}</Text>
                      </Chip>
                    ) : (
                      <Chip
                        variant="Success"
                      ...
                      >
                        <Text size="B300">{t('room_settings.general.address.published.set_main_button')}</Text>
                      </Chip>
                    )}
                  </Box>
                )}
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="200">
      <Box gap="200">
        ...
        <Box shrink="No">
          ...
          >
            <Text size="B400">{t('room_settings.general.address.local.save_button')}</Text>
          </Button>
        </Box>
      </Box>
      {addState.status === AsyncStatus.Error && (
        <Text style={{ color: color.Critical.Main }} size="T200">
          {(addState.error as MatrixError).httpStatus === 409
            ? t('room_settings.general.address.local.already_in_use_error')
    ...
    <Box direction="Column" gap="300">
      {selectedAliases.length > 0 && (
        <Box gap="200">
          <Box grow="Yes">
            <Text size="L400">
              {t('room_settings.general.address.local.selected_count', {
          ...
          <Box shrink="No" gap="Inherit">
            {canEditCanonical &&
              (selectHasPublished ? (
                ...
                >
                  <Text size="B300">{t('room_settings.general.address.local.unpublish_button')}</Text>
                </Chip>
              ) : (
                <Chip
                  variant="Success"
                ...
                >
                  <Text size="B300">{t('room_settings.general.address.local.publish_button')}</Text>
                </Chip>
              ))}
            <Chip
              variant="Critical"
            ...
            >
              <Text size="B300">{t('room_settings.general.address.local.delete_button')}</Text>
            </Chip>
          </Box>
        </Box>
      )}
            ...
            <Box shrink="No" gap="100">
              {published && (
                <Badge variant="Success" fill="Soft" size="500">
                  <Text size="L400">{t('room_settings.general.address.local.published_badge')}</Text>
                </Badge>
              )}
            </Box>
          </Box>
...
export function RoomLocalAddresses({ permissions }: { permissions: RoomPermissionsAPI }) {
  ...
  return (
    ...
    >
      <SettingTile
        title={t('room_settings.general.address.local.title')}
        description={t('room_settings.general.address.local.description')}
        after={
          <Button
            type="button"
          ...
          >
            <Text as="span" size="B300" truncate>
              {expand
                ? t('room_settings.general.address.local.collapse_button')
                : t('room_settings.general.address.local.expand_button')}
            </Text>
          </Button>
        }
      />
        ...
        <CutoutCard variant="Surface" style={{ padding: config.space.S300 }}>
          {localAliasesState.status === AsyncStatus.Loading && (
            <Box gap="100">
              ...
              <Text size="T200">{t('room_settings.general.address.local.loading')}</Text>
            </Box>
          )}
          {localAliasesState.status === AsyncStatus.Success &&
            (localAliasesState.data.length === 0 ? (
              <Box direction="Column" gap="100">
                <Text size="L400">{t('room_settings.general.address.local.empty_title')}</Text>
              </Box>
            ) : (
              <LocalAddressesList
