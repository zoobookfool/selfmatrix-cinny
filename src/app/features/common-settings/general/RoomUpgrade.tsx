          ...
          <Dialog variant="Surface">
            ...
            >
              <Box grow="Yes">
                <Text size="H4">
                  {room.isSpaceRoom()
                    ? t('room_settings.general.upgrade.dialog_title_space')
                    : t('room_settings.general.upgrade.dialog_title_room')}
                </Text>
              </Box>
            ...
            <Box style={{ padding: config.space.S400 }} direction="Column" gap="400">
              <Text priority="400" style={{ color: color.Critical.Main }}>
                <b>{t('room_settings.general.upgrade.irreversible_warning')}</b>
              </Text>
              <Box direction="Column" gap="100">
                <Text size="L400">{t('room_settings.general.upgrade.options_label')}</Text>
                <RoomVersionSelector
              ...
              >
                <Text size="B400">
                  {room.isSpaceRoom()
                    ? t('room_settings.general.upgrade.dialog_submit_button_space')
                    : t('room_settings.general.upgrade.dialog_submit_button_room')}
                </Text>
              </Button>
            </Box>
          </Dialog>
  ...
  return (
    ...
    >
      <SettingTile
        title={
          room.isSpaceRoom()
            ? t('room_settings.general.upgrade.title_space')
            : t('room_settings.general.upgrade.title_room')
        }
        description={
          replacementRoom
            ? tombstoneContent.body ||
              (room.isSpaceRoom()
                ? t('room_settings.general.upgrade.replaced_description_space')
                : t('room_settings.general.upgrade.replaced_description_room'))
            : t('room_settings.general.upgrade.current_version_description', { roomVersion })
        }
        after={
          <Box alignItems="Center" gap="200">
            {predecessorRoomId && (
              ...
              >
                <Text size="B300">
                  {room.isSpaceRoom()
                    ? t('room_settings.general.upgrade.old_space_button')
                    : t('room_settings.general.upgrade.old_room_button')}
                </Text>
              </Button>
            )}
            {replacementRoom ? (
              ...
              >
                <Text size="B300">
                  {room.isSpaceRoom()
                    ? t('room_settings.general.upgrade.open_new_space_button')
                    : t('room_settings.general.upgrade.open_new_room_button')}
                </Text>
              </Button>
            ) : (
              <Button
              ...
              >
                <Text size="B300">{t('room_settings.general.upgrade.upgrade_button')}</Text>
              </Button>
            )}
          </Box>
        }
